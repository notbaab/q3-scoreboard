...
def add_kill(game_id, killer_name, victum_name, method):
    # print(stat_table)
    killer_id = get_user_id_from_username(killer_name)
    victum_id = get_user_id_from_username(victum_name)
    kill_entry = models.GameKill(game_id, killer_id, victum_id, method)
...
def read_game_from_queue(game_start_data, queue, stop_flag):
    ...
    while not stop_flag.stop:
        ...
        if line_type == lineparsers.LineType.PLAYER_INFO:
            ...
            killer_name, victum_name, method = lineparsers.parse_kill(line)
            add_kill(game_id, killer_name, victum_name, method)
            ...
            if player_tracker is None:
                print("something fucky happened, couldn't find %s" % player_name)
                continue
